...
export class ApiServer {
  ...
  constructor(config: ApiServerConfig = { port: 1299 }, databaseManager: DatabaseManager) {
    this.config = {
      host: '0.0.0.0',
      enableCors: true,
      enableHelmet: true,
  ...
  private configureRoutes(): void {
    ...
    // GET /plugins/:id/* - 插件页面托管
    this.app.get('/plugins/:id/*', (req: express.Request, res: express.Response) => {
      const pluginId = req.params.id;
      const resourcePath = req.params[0];
      
      // TODO: 实现插件静态资源托管
      res.status(501).json({
        error: 'Plugin hosting not implemented yet',
        pluginId,
        resourcePath
      });
    });
  ...
  private configureErrorHandling(): void {
    this.app.use((err: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
      console.error('[ApiServer] 错误:', err);
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.server.listen(this.config.port, this.config.host, () => {
          console.log(`[ApiServer] HTTP 服务器运行在 http://${this.config.host}:${this.config.port}`);
          console.log(`[ApiServer] WebSocket 服务器已启动`);
          resolve();
        });

        this.server.on('error', (error) => {
          console.error('[ApiServer] 服务器错误:', error);
          reject(error);
        });

      } catch (error) {
        console.error('[ApiServer] 启动失败:', error);
        reject(error);
      }
    });
  }
