...
import { robotSetting } from "@front/datas/aboutDanmaku";
...
export default defineComponent({
	...
	beforeMount() {
		if (this.danmakuProfile.toolBox.robotSetting) {
			this.robotSetting = Object.assign(this.robotSetting, cloneDeep(this.danmakuProfile.toolBox.robotSetting));
	...
	methods: {
		...
		switchType(on: boolean, type: any) {
			this.robotSetting.enable = this.robotSetting.enable.filter((ty) => ty !== type);
			if (on) {
