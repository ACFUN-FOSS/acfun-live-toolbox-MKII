<template>
	<content-frame id="superChat" align="row">
		...
		<row-frame title=" " style="width: 100%" :flex="true" alignCenter>
			<row-span :span="1"> 开启 </row-span>
			<row-span :span="1">
				<el-switch v-model="superChat.enable" />
			</row-span>
			<template v-if="superChat.enable">
				<row-span :span="1.5"> 观众端开启 </row-span>
				<row-span :span="1">
					<el-switch v-model="superChat.webEnable" />
				</row-span>
				<row-span :span="1.8"> 显示晋级条件 </row-span>
				<row-span :span="1">
...
import { superChat } from "@front/datas/aboutDanmaku";
...
export default defineComponent({
	...
	mounted() {
		this.superChat = cloneDeep(this.danmakuProfile.common.superChat) || superChat();
	},
	methods: {
		save() {
			if (!this.saveCheck()) {
