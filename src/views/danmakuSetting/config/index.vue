...
export default defineComponent({
	...
	methods: {
		polling() {
			clearTimeout(this.mockTimer);
			this.mockData = this.filter.filterUpdate([...getMock(random(0, 3))], this.mockData, this.settings, this.commonSettings, this.tempRank).list;
			this.$nextTick(() => {
		...
		resetFilter() {
			...
			this.mockData = this.filter.filter(newMock, this.settings, this.commonSettings, this.tempRank).filtered;
		},
		getProfile(target: any) {
			this.settings = cloneDeep(this.danmakuProfile[target]);
			this.commonSettings = cloneDeep(this.$store.state.danmakuProfile.common) || commonSettings();
		},
		save() {
