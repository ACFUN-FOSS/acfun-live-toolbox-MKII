<template>
	<template v-if="settingTarget !== 'web'">
		...
		<row-frame :flex="true">
			...
			<row-span :span="3">
				<el-switch
					v-model="setting.filter.clubOnly"
					:disabled="!setting.filter.open"
				/>
			</row-span>
		...
		<row-frame :flex="true">
			...
			<row-span :span="3">
				<el-switch
					:disabled="!setting.filter.open"
					v-model="setting.filter.blackList"
				/>
			</row-span>
			<row-span :span="3" :class="{ disabled: !setting.filter.open }">
				关键词屏蔽
			...
			<row-span :span="3">
				<el-switch
					:disabled="!setting.filter.open"
					v-model="setting.filter.keyword"
				/>
			</row-span>
		</row-frame>
		<row-frame :flex="true">
			...
			<row-span :span="9" v-show="setting.filter.blackList">
				<el-select
					:disabled="!setting.filter.open"
					v-model="setting.filter.types"
					multiple
					style="width: 100%"
					placeholder="多选显示弹幕类型"
				>
					<el-option
	...
	<row-frame title="弹幕合并" :flex="true">
		...
		<row-span :span="3">
			<el-switch
				:disabled="!setting.filter.open"
				v-model="setting.filter.combineGift"
			/>
		</row-span>
		<row-span :span="3" :class="{ disabled: !setting.filter.open }">
			点赞
		...
		<row-span :span="3">
			<el-switch
				:disabled="!setting.filter.open"
				v-model="setting.filter.combineLike"
			/>
		</row-span>
	</row-frame>
	<row-frame title="" :flex="true">
		...
		<row-span :span="3">
			<el-switch
				:disabled="!setting.filter.open"
				v-model="setting.filter.combineEnter"
			/>
		</row-span>
		<div class="hint">
			合并：针对一定时期内相同用户发出的相同类型弹幕进行合并
		</div>
	</row-frame>
</template>
