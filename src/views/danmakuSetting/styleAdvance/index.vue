	...
	<content-frame align="row" class="danmaku-setting" :style="{ '--brightness': brightness / 100 }">
		...
		<row-frame width="100%" title="这是预览">
			...
			<zoom-frame :allow-zoom="zoomPreview" class="zoom-frame" :styles="{ width: '100%', height: '60px' }">
				...
				<danmaku-row style="white-space: nowrap" v-else :setting="currentStyle" :config-mode="true" :danmaku="mockDanmaku" :key="styleType" />
			</zoom-frame>
		</row-frame>
			...
			action: [
				{
					...
					action: (e: any) => {
						...
						const that: any = this;
						editor.value = editor.value.slice(0, that.selectionStart) + e.label + editor.value.slice(that.selectionEnd);
	...
	methods: {
		...
		loadCommand(command: string) {
			const loadFromStore = (type: string) => {
				this.styles = cloneDeep(this.danmakuProfile(type).settingOfType);
				ElMessage({
					message: "读取成功",
					duration: 1500,
					type: "success"
		...
		saveCommand(command: string) {
			...
			switch (command) {
				...
				case "web":
					...
					break;
				case "both":
					this.$store.state.danmakuProfile.web.settingOfType = this.$store.state.danmakuProfile.toolBox.settingOfType = this.removeStyleForm(this.styles);
					this.$store.commit("updateStyle", {});
					break;
				case "file":
					saveToFile();
