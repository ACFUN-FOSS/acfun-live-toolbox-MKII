...
export default defineComponent({
	...
	methods: {
		...
		next() {
			const nextOne = this.temp.superChatArray.find((block: any) => !block?.showed);
			if (!nextOne && this.currentTime > this.temp.superChatBlock.listEndTime) {
		...
		remove(block: any) {
			this.temp.superChatArray = this.temp.superChatArray.filter((blockI: any) => blockI !== block);
