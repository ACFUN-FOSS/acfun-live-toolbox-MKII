import { Settings, CommonSettings } from "@front/datas/aboutDanmaku";
import { Rank } from "@front/datas/room";
import { getContent, getUserInfo, getMedal, getUID } from "./getter";
import { reactive } from "vue";
import { isOwner as ownerTest, isManager as managerTest, sameGift, samePerson } from "./tester";
...
export class Filter {
	...
	likeList: any = [];
	enterList: any = [];
	filter = (danmaku: Array<any>, { filter, maxNum, clubLevel, settingOfType }: Settings, { blackList, keywords }: CommonSettings, rank: Rank): any => {
		...
		for (let i of danmaku) {
			...
			if (filter.types.indexOf(i.type) < 0 || keys.indexOf(String(i.type)) < 0) {
			...
			if (
				...
				filter.blackList &&
				blackList.find((u) => {
					return u === getUserInfo(i).userID || u.userID === getUserInfo(i).userID;
				})
			) {
				hasBlackList = true;
			...
			if (!isOwner && filter.clubOnly && rank.clubName && getMedal(i)?.clubName !== rank.clubName) {
			...
			if (
				...
				const now = Date.now();
				this.giftList = this.giftList.filter((i: any) => now < i.expire);
				const gift = this.giftList.find((g: any) => samePerson(i, g) && sameGift(i, g));
				if (gift) {
	...
	filterUpdate = (changedDanmaku: Array<any>, OldDanmaku: Array<any>, settings: Settings, commonSettings: CommonSettings, rank: Rank) => {
		const result = this.filter(changedDanmaku, settings, commonSettings, rank);
		return {
			list: this.filterMaxnium([...result.filtered.reverse(), ...OldDanmaku], settings),
			...result,
		};
	};
...
export const menu = (danmaku: any, store: Store<any>): Array<rightClickMenuItem> => {
	if (!danmaku) {
		return [];
	}
	...
	const isBlackList = store.state.danmakuProfile.common.blackList.find((i: any) => i.userID === userProfile.userID);
	const likeList = store.state.danmakuProfile.common.likeList || [];
	const isLikeList = likeList.find((i: any) => i.userID === userProfile.userID);
	return [
		{
