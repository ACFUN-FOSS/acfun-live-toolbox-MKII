...
import { BackendMsg, BackendMsgPushDanmakuData as BackendPushDanmakuMsgData, BackendMsgType } from "@front/datas/dto/acfunlive-backend/msg";
// tsdoc imports =====
...
export function getBackendMsgType(resp: BackendMsg<any>): BackendMsgType | undefined {
	return new Map<number, BackendMsgType>([[1000, BackendMsgType.PUSH_DANMAKU]]).get(resp.type);
}
...
export const getContentWithEmoji = function (danmaku: any) {
	...
	if (isElectron()) {
		[...content.matchAll(/(ac|AC|aC|Ac)\d+/g)].forEach((match) => {
			content = Lodash.replace(content, match[0], `<a target="_blank" href="https://www.acfun.cn/v/${match[0]}" style="color:rgb(100,238,238)">${match[0]}(点击查看)</a>`);
		});
	}
	...
	[...content.matchAll(emojiTester)].forEach((match, index) => {
		...
		let replacement = "";
		if (index < max) {
			replacement = `<img style="max-width:${emojiMatcher[match[0]].scale}px;margin-left:8px;margin-top:8px;vertical-align:bottom" src="${emojiMatcher[match[0]].url}" />`;
		}
