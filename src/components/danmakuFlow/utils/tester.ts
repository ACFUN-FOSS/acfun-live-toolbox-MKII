 ...
 */
import { getTime, getMedal, getAvatar, getNickName, getContent, getUserInfo, getGift } from "./getter";
...
export const isManager = function (danmaku: any, state: RootState) {
	...
	return Boolean(state.managerList.find((i) => i.userInfo.userID === getUserInfo(danmaku).userID));
};
...
export const isNormalDanmaku = function (danmaku: any) {
	//@ts-ignore
	if (!danmaku) danmaku = this;
	return [1000, 1001, 1002, 1003, 1004, 1005, 1006, 1007, 1008].includes(danmaku.type);
};

export const sameGift = function (danmakuA: any, danmakuB: any) {
	return getGift(danmakuA).giftDetail.giftID === getGift(danmakuB).giftDetail.giftID;
};
